...
mindmap
  root((Exercícios Fixação))
    Ex1[Tipos de dados, operadores aritméticos e booleanos]
    Ex2[Operador módulo [%], condicionais, ano bissexto]
    Ex3[Listas, filtragem por tipo [int/float]]
    Ex4[Funções, juros compostos, estruturas de repetição]
    Ex5[Fatiamento de listas, comprehension com filtro]
    Ex6[Funções, dicionários, contagem de frequência]
    Ex7[Tuplas, listas aninhadas, média]
    Ex8[Pandas: leitura CSV, DataFrame, index, dtypes]
    Ex9[Pandas: remoção de colunas, filtragem por intervalo]
    Ex10[Pandas: novas colunas, filtragem, agregação]
    Ex11[Comprehension, range com passo float, round[]]
    Ex12[NumPy/math: log, tratamento de zero [nan]]
    Ex13[Comprehension paralela, multiplicação elemento-a-elemento]
    Ex14[Comprehension aninhada, matriz nxn]
    Ex15[Expressões lambda, manipulação de strings]
    Ex16[NumPy: listas de tuplas → array 2D]
    Ex17[NumPy: randint, slicing de submatriz, filtragem]
    Ex18[NumPy: broadcasting, comparação entre arrays]
    Ex19[Pandas: filtragem múltipla, agrupamento]
    Ex20[Pandas: merge/join de DataFrames, limpeza]
```
